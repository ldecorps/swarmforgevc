...
const MODEL_IDS = ['qwen2.5-coder:7b-instruct', 'llama3.1:8b', 'gemma3:4b'];
...
test('BL-1082/BL-654 invariant 2: pull writes stay outside the tracked worktree', () => {
  ...
  assert.equal(plan.environment.OLLAMA_MODELS, modelStorePath);
  assert.equal(plan.writePaths.every((p) => !isPathInside(p, repoRoot)), true);
  assert.match(plan.command || '', new RegExp(escapeRegExp(modelStorePath)));
});

test('BL-1082/BL-654 invariant 3: serving a healthy endpoint reuses it rather than starting a second server', () => {
  fc.assert(
