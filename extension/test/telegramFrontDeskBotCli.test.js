...
const {
  ...
  handleOnboardingFacilitatorMessage,
  ensureControlTopic,
...
// ── handleOnboardingFacilitatorMessage idempotency (BL-590 architect
...
test('BL-590 architect bounce defect 1: the same updateId processed twice writes state once and sends once', async () => {
  ...
  const first = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'https://github.com/acme/widget', 900, postFn);
  assert.equal(first, true);
  assert.equal(calls.length, 1);
  const afterFirst = readOnboardingFacilitatorState(root, 'https://github.com/acme/widget');
  ...
  const second = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'https://github.com/acme/widget', 900, postFn);
  assert.equal(second, true);
  assert.equal(calls.length, 1, 'expected no second send for a redelivered updateId');
  assert.deepEqual(readOnboardingFacilitatorState(root, 'https://github.com/acme/widget'), afterFirst, 'expected no second write for a redelivered updateId');
});
...
test('BL-590 architect bounce defect 1: redelivery of the fork-clone verification never re-applies as a failing verification for the NEXT step', async () => {
  ...
  writeOnboardingFacilitatorState(root, state);

  const forkCloneOutput = "Cloning into 'swarm-forge'...";
  const ok = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, forkCloneOutput, 901, postFn);
  assert.equal(ok, true);
  assert.deepEqual(readOnboardingFacilitatorState(root, 'https://github.com/acme/widget').verifiedSteps, ['toolchain', 'github-access', 'fork-clone']);
  assert.equal(calls.length, 1);
  ...
  const redelivered = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, forkCloneOutput, 901, postFn);
...
test('BL-590 architect bounce defect 1: a DIFFERENT updateId is not short-circuited by an unrelated updateId already on record', async () => {
  ...
  const ok = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'proceed', 901, postFn);
  assert.equal(ok, true);
...
// ── Reproductions D1/D2 from the architect's SEND BACK #3 evidence
// (backlog/evidence/BL-590-facilitator-slice1-architect-bounce3-20260725.md)
// - bounce #2's guard exempted the 'no-active-onboarding' branch on the
// premise that a redelivery there just "recomputes the same constant
// message". False: WHICH branch runs is decided by listOnboardingFacilitatorStates,
// durable state a LATER update in the same getUpdates batch can mutate (a
// target starting in between). These pin the redelivered update to the
// ORIGINAL no-active reply, never a re-run of handleOnboardingMessage
// against the onboarding that has since started. ─────────────────────────

test('BL-590 architect bounce #3, Reproduction D1: a failed no-active-onboarding reply is retried on redelivery, never re-applied as a verification once a target has since started', async () => {
  const root = mkTmpRoot();
  const { postFn, calls } = fakeSendFailThenOk(1);

  const first = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'hi', 700, postFn);
  assert.equal(first, false, 'the failed send must be reported as a failure');
  assert.equal(calls.length, 1);

  const started = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'https://github.com/acme/widget', 701, postFn);
  assert.equal(started, true);
  assert.equal(calls.length, 2);
  const afterStart = readOnboardingFacilitatorState(root, 'https://github.com/acme/widget');
  assert.deepEqual(afterStart.verifiedSteps, []);

  // u700 redelivered: must retry the ORIGINAL "no onboarding in progress"
  // reply, never re-run against the onboarding u701 just started.
  const redelivered = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'hi', 700, postFn);
  assert.equal(redelivered, true, 'the retry must actually be attempted, and must succeed');
  assert.equal(calls.length, 3);
  assert.doesNotMatch(calls[2].body, /verification failed/i, 'must not misapply stale text against the just-started onboarding');
  assert.match(calls[2].body, /No onboarding is currently in progress/);
  assert.deepEqual(readOnboardingFacilitatorState(root, 'https://github.com/acme/widget'), afterStart, 'must not mutate the freshly started onboarding state');
});

test('BL-590 architect bounce #3, Reproduction D2: a redelivered no-active-onboarding update does not regress a target started in between', async () => {
  const root = mkTmpRoot();
  const { postFn, calls } = fakeSendFailThenOk(1);

  const first = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'pause', 700, postFn);
  assert.equal(first, false);
  assert.equal(calls.length, 1);

  await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'https://github.com/acme/widget', 701, postFn);
  assert.equal(calls.length, 2);
  assert.equal(readOnboardingFacilitatorState(root, 'https://github.com/acme/widget').paused, false);

  const redelivered = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'pause', 700, postFn);
  assert.equal(redelivered, true);
  assert.equal(calls.length, 3);
  assert.equal(readOnboardingFacilitatorState(root, 'https://github.com/acme/widget').paused, false, 'must not silently re-pause the just-started onboarding');
});

test('BL-590 architect bounce #3: a redelivered no-active-onboarding updateId is recognised by the SAME guard as a per-target one', async () => {
  const root = mkTmpRoot();
  const { postFn, calls } = fakeSendOk(1);
  await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'hello', 800, postFn);
  assert.equal(calls.length, 1);
  assert.equal(hasProcessedOnboardingUpdateId(root, 800), true);

  const redelivered = await handleOnboardingFacilitatorMessage(root, 'fake-token', 'fake-chat', 42, 'hello', 800, postFn);
  assert.equal(redelivered, true);
  assert.equal(calls.length, 1, 'expected no second send for a redelivered no-active-onboarding updateId');
});
