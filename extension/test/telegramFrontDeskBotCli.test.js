...
// ── handleOnboarderMessage idempotency (BL-590 architect
// bounce, defect 1, 2026-07-25) - a redelivered updateId (the offset only
// advances after processing, so a crash between the state write and the
// offset commit makes Telegram re-serve the same update) used to advance
// the wrong step and emit a spurious failure plus a duplicate post. Mirrors
// the openSubjectAndRecord/postOperatorContext idempotency tests above:
// prove the SECOND call with the same updateId short-circuits before either
// the durable write or the send, not merely that it "would" behave that
// way. ──────────────────────────────────────────────────────────────────

test('BL-590 architect bounce defect 1: the same updateId processed twice writes state once and sends once', async () => {
  const root = mkTmpRoot();
  const { postFn, calls } = fakeSendOk(1);

  const first = await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'https://github.com/acme/widget', 900, postFn);
  assert.equal(first, true);
  assert.equal(calls.length, 1);
  const afterFirst = readOnboarderState(root, 'https://github.com/acme/widget');
  assert.equal(afterFirst.phase, 'checking-prerequisites');

  // Redelivery: SAME updateId, and (as a real redelivery would) the exact
  // same text - must be a total no-op, not merely idempotent-by-luck.
  const second = await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'https://github.com/acme/widget', 900, postFn);
  assert.equal(second, true);
  assert.equal(calls.length, 1, 'expected no second send for a redelivered updateId');
  assert.deepEqual(readOnboarderState(root, 'https://github.com/acme/widget'), afterFirst, 'expected no second write for a redelivered updateId');
});

test('BL-590 architect bounce defect 1: redelivery of the fork-clone verification never re-applies as a failing verification for the NEXT step', async () => {
  const root = mkTmpRoot();
  const { postFn, calls } = fakeSendOk(1);
  let state = createOnboardingState('https://github.com/acme/widget', () => 1_700_000_000_000);
  state = { ...state, phase: 'checking-prerequisites', stepIndex: 2, verifiedSteps: ['toolchain', 'github-access'] };
  writeOnboarderState(root, state);

  const forkCloneOutput = "Cloning into 'swarm-forge'...";
  const ok = await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, forkCloneOutput, 901, postFn);
  assert.equal(ok, true);
  assert.deepEqual(readOnboarderState(root, 'https://github.com/acme/widget').verifiedSteps, ['toolchain', 'github-access', 'fork-clone']);
  assert.equal(calls.length, 1);
  assert.doesNotMatch(calls[0].body, /verification failed/i);

  // Redelivered: same updateId, same (now stale) text - must NOT be
  // re-applied against the now-current "target-repo" step.
  const redelivered = await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, forkCloneOutput, 901, postFn);
  assert.equal(redelivered, true);
  assert.equal(calls.length, 1, 'expected no spurious failure message for the redelivered update');
  assert.deepEqual(readOnboarderState(root, 'https://github.com/acme/widget').verifiedSteps, ['toolchain', 'github-access', 'fork-clone']);
});

test('BL-590 architect bounce defect 1: a DIFFERENT updateId is not short-circuited by an unrelated updateId already on record', async () => {
  const root = mkTmpRoot();
  const { postFn, calls } = fakeSendOk(1);
  await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'https://github.com/acme/widget', 900, postFn);
  assert.equal(calls.length, 1);

  const ok = await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'proceed', 901, postFn);
  assert.equal(ok, true);
...
test('BL-590 architect bounce #2: hasProcessedOnboardingUpdateId remembers EVERY processed updateId, not only the most recent', async () => {
  ...
  await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'https://github.com/acme/widget', 900, postFn);
  await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'proceed', 901, postFn);
...
// (backlog/evidence/BL-590-onboarder-slice1-architect-bounce2-20260725.md)
...
test('BL-590 architect bounce #2, Reproduction A: redelivering an update OLDER than the most recent one is still a no-op', async () => {
  ...
  await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'https://github.com/acme/widget', 100, postFn);
  await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, toolchainOutput, 101, postFn);
  await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, githubOutput, 102, postFn);
  assert.equal(calls.length, 3);
  const afterStep102 = readOnboarderState(root, 'https://github.com/acme/widget');
  ...
  const redelivered = await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, toolchainOutput, 101, postFn);
  assert.equal(redelivered, true);
  assert.equal(calls.length, 3, 'expected no spurious failure message for the redelivered OLDER update');
  assert.deepEqual(readOnboarderState(root, 'https://github.com/acme/widget'), afterStep102, 'expected no state change from the redelivered OLDER update');
});

test('BL-590 architect bounce #2, Reproduction B: a stale redelivered control word does not regress durable state', async () => {
  const root = mkTmpRoot();
  const { postFn, calls } = fakeSendOk(1);
  writeOnboarderState(root, createOnboardingState('https://github.com/acme/widget', () => 1_700_000_000_000));

  await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'pause', 200, postFn);
  assert.equal(readOnboarderState(root, 'https://github.com/acme/widget').paused, true);
  await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'proceed', 201, postFn);
  assert.equal(readOnboarderState(root, 'https://github.com/acme/widget').paused, false);
  assert.equal(calls.length, 2);

  // The human explicitly resumed (201). A redelivery of the now-stale 200
  // ("pause") must not silently re-pause the onboarder.
  const redelivered = await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'pause', 200, postFn);
  assert.equal(redelivered, true);
  assert.equal(calls.length, 2, 'expected no second "paused" message from the redelivered stale control word');
  assert.equal(readOnboarderState(root, 'https://github.com/acme/widget').paused, false, 'must not regress to paused');
});
...
test('BL-590 architect bounce #2, Reproduction C: a failed send followed by redelivery ends with the human receiving exactly one reply', async () => {
  ...
  const first = await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, toolchainOutput, 300, postFn);
  ...
  assert.deepEqual(readOnboarderState(root, 'https://github.com/acme/widget').verifiedSteps, ['toolchain']);
  assert.equal(findProcessedOnboardingUpdate(root, 300).record.delivered, false);

  const redelivered = await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, toolchainOutput, 300, postFn);
  ...
  assert.deepEqual(readOnboarderState(root, 'https://github.com/acme/widget').verifiedSteps, ['toolchain'], 'must not re-apply the verification a second time');
  assert.equal(findProcessedOnboardingUpdate(root, 300).record.delivered, true);
});

test('BL-590 slice 1: when no onboarding is in progress, the onboarder tells the user to start one with a repo URL', async () => {
  const root = mkTmpRoot();
  const { postFn, calls } = fakeSendOk(1);
  const result = await handleOnboarderMessage(root, 'fake-token', 'fake-chat', 42, 'hello', 500, postFn);
...
// ── BL-846: resolveRolePaneTarget follows the mono-router resident-identity
// marker for the role it names, so a rotating role IS reachable while it is
// the active resident - every other role/marker state resolves exactly as
// before. ──────────────────────────────────────────────────────────────────

function writeMonoRouterSwarmFixture(root, roles) {
  const stateDir = path.join(root, '.swarmforge');
  fs.mkdirSync(stateDir, { recursive: true });
  fs.writeFileSync(path.join(stateDir, 'tmux-socket'), '/tmp/fake.sock');
  const lines = roles.map((role, i) => `${i + 1}\t${role}\tswarmforge-${role}\t${role}\tclaude`).join('\n');
  fs.writeFileSync(path.join(stateDir, 'sessions.tsv'), `${lines}\n`);
}

function writeMonoRouterActiveRoleMarker(root, role) {
  fs.writeFileSync(path.join(root, '.swarmforge', 'mono-router-active-role'), `${role}\n`);
}

test('BL-846: resolveRolePaneTarget resolves the RESIDENT pane for a role the active-identity marker names', () => {
  const root = mkTmpRoot();
  writeMonoRouterSwarmFixture(root, ['coder', 'QA', 'coordinator']);
  writeMonoRouterActiveRoleMarker(root, 'QA');
  const fake = installInProcessTmux([{ subcommand: 'show-window-options', exitCode: 0, stdout: '1\n' }]);
  try {
    const resolved = resolveRolePaneTarget(root, 'QA');
    assert.deepEqual(
      resolved,
      { socketPath: '/tmp/fake.sock', target: 'swarmforge-coder:coder.1' },
      "expected QA's answer resolved into the resident (coder) pane, not QA's own never-created session"
    );
  } finally {
    fake.restore();
  }
...
test('BL-846: resolveRolePaneTarget leaves a role NOT named by the marker resolving to its own session', () => {
  const root = mkTmpRoot();
  writeMonoRouterSwarmFixture(root, ['coder', 'QA', 'cleaner', 'coordinator']);
  writeMonoRouterActiveRoleMarker(root, 'QA');
  const fake = installInProcessTmux([{ subcommand: 'show-window-options', exitCode: 0, stdout: '1\n' }]);
  try {
    const resolved = resolveRolePaneTarget(root, 'cleaner');
    assert.deepEqual(
      resolved,
      { socketPath: '/tmp/fake.sock', target: 'swarmforge-cleaner:cleaner.1' },
      "expected cleaner's own session, unaffected by QA being the named resident"
    );
  } finally {
    fake.restore();
  }
...
test('BL-846: resolveRolePaneTarget never redirects the coordinator to the resident pane', () => {
  const root = mkTmpRoot();
  writeMonoRouterSwarmFixture(root, ['coder', 'QA', 'coordinator']);
  writeMonoRouterActiveRoleMarker(root, 'QA');
  const fake = installInProcessTmux([{ subcommand: 'show-window-options', exitCode: 0, stdout: '1\n' }]);
  try {
    const resolved = resolveRolePaneTarget(root, 'coordinator');
    assert.deepEqual(resolved, { socketPath: '/tmp/fake.sock', target: 'swarmforge-coordinator:coordinator.1' });
  } finally {
    fake.restore();
  }
});

test('BL-846: resolveRolePaneTarget resolves the role\'s own session when the active-identity marker is missing', () => {
  const root = mkTmpRoot();
  writeMonoRouterSwarmFixture(root, ['coder', 'QA', 'coordinator']);
  const fake = installInProcessTmux([{ subcommand: 'show-window-options', exitCode: 0, stdout: '1\n' }]);
  try {
    const resolved = resolveRolePaneTarget(root, 'QA');
    assert.deepEqual(resolved, { socketPath: '/tmp/fake.sock', target: 'swarmforge-QA:QA.1' });
  } finally {
    fake.restore();
  }
});

test('BL-846: resolveRolePaneTarget resolves the role\'s own session when the active-identity marker is blank', () => {
  const root = mkTmpRoot();
  writeMonoRouterSwarmFixture(root, ['coder', 'QA', 'coordinator']);
  writeMonoRouterActiveRoleMarker(root, '   ');
  const fake = installInProcessTmux([{ subcommand: 'show-window-options', exitCode: 0, stdout: '1\n' }]);
  try {
    const resolved = resolveRolePaneTarget(root, 'QA');
    assert.deepEqual(resolved, { socketPath: '/tmp/fake.sock', target: 'swarmforge-QA:QA.1' });
...
test('BL-846: resolveRolePaneTarget resolves the RESIDENT pane for "coder" itself when the marker names coder', () => {
  const root = mkTmpRoot();
  writeMonoRouterSwarmFixture(root, ['coder', 'coordinator']);
  writeMonoRouterActiveRoleMarker(root, 'coder');
  const fake = installInProcessTmux([{ subcommand: 'show-window-options', exitCode: 0, stdout: '1\n' }]);
  try {
    const resolved = resolveRolePaneTarget(root, 'coder');
    assert.deepEqual(resolved, { socketPath: '/tmp/fake.sock', target: 'swarmforge-coder:coder.1' });
