...
test('runConciergeTick: sweepStaleApprovalAsks runs before folder diff when wired', async () => {
  const calls = [];
  const { adapters, setFolders } = fakeAdapters({
  ...
  setFolders(folders());
  await runConciergeTick(adapters, 4242);
...
test('BL-467: the pin sync follows a board repost to the new message id with unpin-all when the old board is still top', async () => {
  ...
  adapters.boardAdapters = {
    ...
    postMessage: async () => ({ messageId: nextMessageId }),
    deleteMessage: async () => true,
  };
  adapters.boardPinAdapters = {
    getTopPinnedMessageId: async () => (nextMessageId === 101 ? 100 : undefined),
