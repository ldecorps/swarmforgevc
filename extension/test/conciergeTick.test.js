...
const { formatUpdatedAtLabel, wrapPipelineBoardHtml } = require('../out/concierge/pipelineBoard');
...
test('BL-462: the pipeline board reposts at the bottom (delete + post) on a stage change, and is a complete no-op - including the footer time - when unchanged', async () => {
  ...
  assert.ok(
    posted[0].text.endsWith(`updated at ${formatUpdatedAtLabel(T1)}`),
    `expected a footer stamped with T1, got:\n${posted[0].text}`
  );
  ...
  assert.ok(
    posted[1].text.endsWith(`updated at ${formatUpdatedAtLabel(T2)}`),
    `expected the footer bumped to T2, got:\n${posted[1].text}`
  );
