...
// one) - the fixture must still be swept either way, since the sweep's
// own job starts only once the creator is already gone.
const deathSignalArb = fc.constantFrom('SIGKILL', 'SIGTERM');

test('property (invariant 2): a fixture is cleared whatever signal killed its creator', () => {
  fc.assert(
    fc.property(deathSignalArb, (signal) => {
      const creatorPid = deadPidVia(signal);
      const name = nameWithCreator(creatorPid);
      const pid = spawnFakeCloudflared(name);
      try {
        const selected = leakedFixtureTunnelPids(execFileSync);
        assert.ok(selected.includes(pid), `expected pid ${pid} (creator died via ${signal}) to be selected, got: ${JSON.stringify(selected)}`);
      } finally {
        killPid(pid);
      }
    }),
