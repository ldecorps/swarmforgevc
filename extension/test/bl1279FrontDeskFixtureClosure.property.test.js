...
// BL-1585: the per-cell run count expressed through runsPerCell rather than
...
describe('BL-1279 invariant 2: a fixture that cannot load reports no passed check', () => {
  ...
  it('refuses and names the file, for every member of the closure', () => {
    ...
    // is removed in turn rather than sampled. BL-1585: the per-member run
    // count expressed through runsPerCell rather than a bare literal, budget
    // unchanged (1 per member).
    ...
    assertReachFloor(coverage, closure, CLOSURE_CELL_RUNS, 'closure member removed');
  });
});
