...
//
// GENERATOR REACH (by construction). The corner that matters is exactly one
// combination - unconfigured WITH something waiting - so each transport gets
// its own property pass and the waiting set is generated within it; the run
// fails unless every combination was reached.
...
test('BL-1352/BL-654 invariant 1: nothing reads healthy while a question sits undelivered', () => {
  ...
  for (const transport of ['configured', 'unconfigured']) {
    fc.assert(
      fc.property(fc.uniqueArray(fc.constantFrom(...ROLES), { maxLength: 3 }), (waiting) => {
