...
test('periodicsync with the expected tag re-fetches and caches every data artifact (dashboard-06)', async () => {
  let fetchCount = 0;
  ...
  assert.equal(fetchCount, 2, 'every data artifact must be re-fetched on periodic sync');
  const cache = await sandbox.caches.open('swarmforge-dashboard-v2');
