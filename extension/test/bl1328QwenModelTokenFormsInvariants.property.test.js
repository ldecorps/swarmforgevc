...
test('BL-1328/BL-654 invariant 3: the asymmetry is documented at both sites and neither precedence moved', () => {
  ...
  // And measured against this parcel's own diff rather than asserted in
  // prose: whatever BL-1328 changed in swarmforge.sh, none of it may be a
  // branch reordering. The only executable change this ticket authorizes is
  // inside extra_cli_targets_qwen_cloud.
  const commits = execFileSync('git', ['log', '--format=%H', '--grep', 'BL-1328', 'origin/main..HEAD'], {
    cwd: REPO_ROOT,
    encoding: 'utf8',
  })
    .split('\n')
  ...
  const added = commits
    .flatMap((sha) =>
      execFileSync('git', ['show', '--first-parent', '--format=', '-U0', sha, '--', 'swarmforge/scripts/swarmforge.sh'], {
        cwd: REPO_ROOT,
        encoding: 'utf8',
      }).split('\n'),
    )
    .filter((line) => /^[+-]/.test(line) && !/^[+-][+-]/.test(line));
  const executable = added.filter((line) => !/^[+-]\s*#/.test(line) && line.slice(1).trim().length > 0);
  for (const line of executable) {
    assert.match(
      line,
      /--model=qwen\*|return 0|if \[\[ "\$\{parts\[i\]\}"|^\+\s*fi$/,
      `BL-1328 changed executable swarmforge.sh code outside the detection helper: ${line}`,
    );
  }
});
