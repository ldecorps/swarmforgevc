...
const { spawnZombie } = require('./fixtureLiveness');
...
// BL-1287 bounce (architect, 2026-09-05): a SIGKILLed process that has not
// yet been reaped answers `process.kill(pid, 0)` as alive, but is
// functionally dead - the exact "however that run died" case invariant 2
// promises to handle. isProcessAlive must treat it as gone.
test('isProcessAlive answers false for a zombie process (BL-1292 unreaped-corpse window)', async () => {
  const zombie = await spawnZombie('bl1287-zombie-isprocessalive-test');
  try {
    assert.equal(zombie.confirmedZombie, true, 'expected a genuine zombie (/proc State: Z) before asserting on it');
