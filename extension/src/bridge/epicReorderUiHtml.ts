...
/// {id, direction} and refreshes. Empty state ("No epics to reorder") when
/// there are none. Polls GET /epic-reorder-state?token=... for the list.
    ...
    fetch('/epic-reorder/move' + q, {
      ...
      return r.json().then(function (payload) {
        if (payload && payload.success) {
          ...
          setStatus(payload && payload.reason ? String(payload.reason) : 'Move failed');
        }
      });
    }).catch(function (err) {
      loading = false;
