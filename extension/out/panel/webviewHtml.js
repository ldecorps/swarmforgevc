...
exports.getWorkTreeHtml = getWorkTreeHtml;
...
function getWorkTreeHtml(nonce) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
function getWebviewHtml(scriptUri, cspSource) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* BL-077: one stable color per pipeline stage, shared by the
       tile-header badge (.tile-bl-badge) and the BACKLOG row chip
       (.bl-assigned) so both surfaces always agree on the color for
       whichever stage currently holds a parcel. Ordered along the
       ...
       Color is additive only - the badge/chip text already names the
       role (bl-assigned's textContent, or the enclosing tile's
       data-role), so color never carries the holder alone. Class names
       are generated by panel.js's stageColorClass(). */
