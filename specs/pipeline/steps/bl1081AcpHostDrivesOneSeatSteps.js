...
(println (json/generate-string
  ...
  define(/^the ACP dimension is a field on that same table$/, (ctx) => {
    ...
    // QA bounce D1: the production launcher must name the ACP host for the
    // spike seat. Grep the live write_role_launch_script, not a fixture.
    const launcher = readSource(path.join(REPO_ROOT, 'swarmforge', 'scripts', 'swarmforge.sh'));
    assert.ok(
      launcher.includes('acp-host-pane.js'),
      'write_role_launch_script never names acp-host-pane.js — the host is still a dark module'
    );
    assert.ok(
      /vibe\)[\s\S]*?acp-host-pane\.js[\s\S]*?;;/.test(launcher) ||
        launcher.includes("--agent vibe") && launcher.includes('acp-host-pane.js'),
      'the vibe branch must launch via acp-host-pane.js'
    );
  });
}
