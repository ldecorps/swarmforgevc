...
// BL-1468: step handlers for "The Stryker mutation gate deferred on
// BL-1452 is run and discharged". Both scenarios read the parcel's OWN
...
// "the contract at this commit"), same shape as BL-1439/BL-1441. Drives
// the real bb CLIs (hardening_debt_ledger_read.bb,
...
const FEATURE = 'BL-1468 The Stryker mutation gate deferred on BL-1452 is run and discharged';
...
function registerSteps(registry) {
  ...
  scoped(/^the ledger row for parcel BL-1452, gate mutation, carries a discharged_at date and a discharged_evidence path$/, (ctx) => {
    const row = ctx.ledgerRows.find((r) => r.parcel === 'BL-1452' && r.gate === 'mutation');
    assert.ok(row, 'expected a ledger row for parcel=BL-1452 gate=mutation');
  ...
  scoped(/^the register report holds no hardening lane row naming BL-1468$/, (ctx) => {
    const hardeningRows = ctx.registerReport.rows.filter((r) => r.lane === 'hardening' && r.ticket === 'BL-1468');
    assert.deepEqual(hardeningRows, [], `the register still carries a hardening row naming BL-1468: ${JSON.stringify(hardeningRows)}`);
  });

  // ── scenario 02 ──────────────────────────────────────────────────────
  scoped(/^the discharge evidence for the mutation gate of BL-1452 is read$/, (ctx) => {
    const rows = readLedgerRows();
    const row = rows.find((r) => r.parcel === 'BL-1452' && r.gate === 'mutation');
    assert.ok(row, 'no ledger row found for parcel=BL-1452 gate=mutation');
    assert.ok(row.discharged_evidence, `BL-1452's mutation gate carries no discharge evidence yet (discharged_at=${row.discharged_at})`);
    ctx.evidencePath = path.join(REPO_ROOT, row.discharged_evidence);
    ctx.evidenceText = fs.readFileSync(ctx.evidencePath, 'utf8');
  });

  scoped(/^it records a completed Stryker run over the three files with zero surviving mutants or a reason per survivor$/, (ctx) => {
  ...
  scoped(/^it records the host load and the duration of the run$/, (ctx) => {
