...
const { buildNamedModelPullPlan, buildNamedModelServePlan, isNamedModelHealthy, formatNamedModelStatus } = require(path.join(
  __dirname,
  '..',
  '..',
  '..',
...
const { isPathInside } = require(path.join(__dirname, '..', '..', '..', 'extension', 'out', 'util', 'pathContainment'));
...
function registerSteps(registry) {
  ...
  define(/^a host model store configured outside the tracked worktree$/, (ctx) => {
    ...
    ctx.availableModels = ['qwen2.5-coder:7b-instruct', 'llama3.1:8b'];
  ...
  define(/^a pull is requested for model "([^"]+)"$/, (ctx, modelId) => {
    ...
    try {
      ctx.pullPlan = buildNamedModelPullPlan(modelId, {
        ...
        presentModelIds: ctx.presentModels,
        availableModelIds: ctx.availableModels,
      });
  ...
  define(/^the composed pull names model "([^"]+)"$/, (ctx, modelId) => {
    assert.ok(ctx.pullPlan, 'expected a composed pull plan, not a failure');
    assert.equal(ctx.pullPlan.modelId, modelId);
    assert.match(ctx.pullPlan.command || '', new RegExp(`ollama pull .*${modelId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`));
  });

  define(/^the pull targets the host model store$/, (ctx) => {
    assert.ok(ctx.pullPlan, 'expected a pull plan, not a failure');
  ...
  define(/^the failure names model "([^"]+)"$/, (ctx, modelId) => {
    assert.match(String(ctx.pullFailure?.message || ctx.pullFailure || ''), new RegExp(modelId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
  });
  ...
  define(/^no path the pull wrote is tracked by git$/, (ctx) => {
    ...
    assert.equal(ctx.pullPlan.writePaths.every((p) => !isPathInside(p, ctx.repoRoot)), true);
  });
}
