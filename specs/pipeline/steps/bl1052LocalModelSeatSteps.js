...
function registerSteps(registry) {
  ...
  registry.define(/^it targets the local inference endpoint on the loopback interface$/, (ctx) => {
    assert.match(
      ctx.launchScript,
      /127\.0\.0\.1|localhost/,
  ...
  registry.define(/^a credential for the local endpoint is exported in the launching environment$/, (ctx) => {
  ...
  registry.define(/^the local endpoint health check reports not ready$/, (ctx) => {
    ctx.endpointStatus = 'missing';
    ctx.endpointUrl = DEFAULT_LOOPBACK_ENDPOINT;
  });

  registry.define(/^a "([^"]+)" window is launched$/, (ctx, agent) => {
    const root = makeFixtureRoot();
  ...
  registry.define(/^the refusal names the endpoint that was not ready$/, (ctx) => {
    assert.match(
      ctx.launchRefusal,
      /127\.0\.0\.1:11434/,
  ...
  registry.define(/^the new local-model pack file is read$/, (ctx) => {
  ...
  registry.define(/^every role window names agent "([^"]+)"$/, (ctx, agent) => {
  ...
  registry.define(/^it requires no cloud provider API key$/, (ctx) => {
    assert.doesNotMatch(
      ctx.packText,
