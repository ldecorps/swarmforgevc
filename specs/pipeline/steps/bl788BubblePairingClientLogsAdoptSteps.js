...
// HTTP / direct calls, mirroring bl851SideloadApkPreauthSteps.js's own
// withBridge(target, fn) try/finally pattern - the bridge is started and
// stopped around each request-issuing step, never left for a later
// happy-path assertion step to stop. An earlier version of this file
// started the bridge in Background and stopped it as the LAST assertion of
// each scenario; when a mid-scenario assertion threw (exactly what a
// killed mutant produces, and what a genuine regression would produce too)
// that stop() was never reached, leaking the listening server and hanging
// `node --test` forever waiting for the event loop to drain. Hardener
// pass, 2026-08-14. Containment against the whole input space (invariant
...
function registerSteps(registry) {
  // ── Background ──────────────────────────────────────────────────────
  registry.define(/^a bridge server is running with a pairing token configured$/, (ctx) => {
    ctx.target = mkTmp();
  ...
  registry.define(/^an unauthenticated client requests the APK path "(.+)"$/, async (ctx, requested) => {
    await withBridge(ctx.target, async (handle) => {
      const res = await rawRequest(handle.port, requested);
      ctx.status = res.status;
