...
function registerSteps(registry) {
  ...
  scoped(/^a residual-word scan with an allowlist of grandfathered files$/, (ctx) => {
    ctx.allowlistOpts = {
  ...
  scoped(/^a grandfathered ticket file allowlisted by its filename$/, (ctx) => {
    ctx.grandfatheredBasename = GRANDFATHERED_BASENAME;
  });

  scoped(/^the ticket sits in (active|paused|hold)$/, (ctx, stage) => {
  ...
  scoped(/^the ticket moves to another stage directory$/, (ctx) => {
  ...
  scoped(/^a ticket file carrying the retired word that is not on the allowlist$/, (ctx) => {
    ctx.extraMatches = [...(ctx.extraMatches ?? []), UNGRANDFATHERED_PATH];
  });

  scoped(/^a grandfathered file allowlisted by its filename$/, (ctx) => {
    ctx.grandfatheredBasename = GRANDFATHERED_BASENAME;
    ctx.grandfatheredPath = backlogStagePath('paused', ctx.grandfatheredBasename);
  });

  scoped(/^a grandfathered file allowlisted by exact path$/, (ctx) => {
  ...
  scoped(/^a different file with the same basename at outside the backlog$/, (ctx) => {
  ...
  scoped(/^a different file with the same basename at elsewhere in the tree$/, (ctx) => {
    ctx.conflictPath = `specs/tmp/${conflictBasename(ctx)}`;
    ctx.extraMatches = [...(ctx.extraMatches ?? []), ctx.grandfatheredPath, ctx.conflictPath];
  });

  scoped(/^an allowlist entry naming a file that no longer exists$/, (ctx) => {
  ...
  scoped(/^the scan reports it as an unexpected match$/, (ctx) => {
    if (ctx.unexpected === undefined) {
      ctx.unexpected = scanUnexpected(ctx.extraMatches ?? [], ctx.allowlistOpts);
    }
