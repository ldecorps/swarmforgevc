...
`;
  ...
  scoped(
    ...
    (ctx) => {
      const st = ensureState(ctx);
      if (st.loadOut.includes('EOF while reading')) {
        throw new Error(`Babashka parse failure loading handoffd.bb: ${st.loadOut}`);
      }
      if (/Phase:\s*parse/i.test(st.loadOut)) {
        throw new Error(`Babashka parse phase error: ${st.loadOut}`);
      }
      if (st.loadStatus !== 0) {
        throw new Error(`expected load to succeed; got ${st.loadStatus}: ${st.loadOut}`);
      }
    }
  );
  ...
  scoped(/^handoffd exits without a load-time syntax failure$/, (ctx) => {
    const st = ensureState(ctx);
    if (st.lastRun.out.includes('EOF while reading')) {
      throw new Error(`parse failure on ${st.lastFlag}: ${st.lastRun.out}`);
    }
    if (st.lastRun.status !== 0) {
      throw new Error(`expected exit 0 for ${st.lastFlag}; got ${st.lastRun.status}`);
    }
  });
  ...
  scoped(/^it states whether the one-shot flag bug is fixed on main today$/, (ctx) => {
    const text = ensureState(ctx).evidenceText;
  ...
  scoped(
    ...
    (ctx) => {
      ...
      if (
        !/6a2e4aaf6/.test(st.evidenceText) ||
        !/deliver!/.test(st.evidenceText) ||
        !/(did not touch|does not appear|paren fix absent|not in its patch)/i.test(st.evidenceText)
      ) {
  ...
  scoped(
    ...
    (ctx) => {
      ...
      st.failedFlags = [];
      for (const { flag, doneLine } of ONE_SHOT_ROWS) {
        const run = runHandoffd(st.root, flag);
        const hay = `${run.out}${run.logText}`;
        if (run.status !== 0 || !hay.includes(doneLine) || run.out.includes('EOF while reading')) {
          st.failedFlags.push(flag);
        }
      }
  ...
  scoped(/^the evidence file records the repair commit$/, (ctx) => {
    const st = ensureState(ctx);
    if (!st.needsRepair) {
      if (!/bug IS fixed on current main/i.test(st.evidenceText || '')) {
        const text = fs.readFileSync(
          findLatestEvidenceFile(EVIDENCE_DIR, EVIDENCE_PREFIX, 'BL-728'),
          'utf8'
        );
        if (!/bug IS fixed on current main/i.test(text)) {
