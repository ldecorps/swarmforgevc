...
const { parseRolesTsv, mailboxDir } = require(path.join(EXT_DIR, 'out', 'swarm', 'swarmState'));
const { mkSocketFixtureRoot } = require('./lib/socketFixtureRoot');
const { copyLiveScriptClosureInto } = require(path.join(EXT_DIR, 'test', 'helpers', 'pinnedRepoFixture'));
...
function seedGitRepo(targetPath) {
  execFileSync('git', ['init', '-q', '-b', 'main'], { cwd: targetPath, env: gitEnv() });
  ...
  copyLiveScriptClosureInto(path.join(targetPath, 'swarmforge', 'scripts'), ['swarm_handoff.bb']);
  execFileSync(
    'git',
    ['-c', 'user.email=t@t', '-c', 'user.name=t', '-c', 'core.hooksPath=/dev/null', 'add', '-A'],
    { cwd: targetPath, env: gitEnv() }
  );
  execFileSync(
    'git',
    ['-c', 'user.email=t@t', '-c', 'user.name=t', '-c', 'core.hooksPath=/dev/null', 'commit', '-q', '-m', 'seed'],
    { cwd: targetPath, env: gitEnv() }
  );
}
...
function listNoteParcels(targetPath, role = DECLARED_ROLE) {
  const dirs = [
    ...
    path.join(targetPath, '.swarmforge', 'handoffs', 'coordinator', 'outbox'),
  ];
  try {
    const roles = parseRolesTsv(fs.readFileSync(path.join(targetPath, '.swarmforge', 'roles.tsv'), 'utf8'));
    const entry = roles.find((item) => item.role === role);
    if (entry) {
      dirs.push(mailboxDir(entry, 'inbox', 'new'));
    }
  } catch {
    dirs.push(path.join(targetPath, '.swarmforge', 'handoffs', 'inbox', 'new'));
  }
  const parcels = [];
