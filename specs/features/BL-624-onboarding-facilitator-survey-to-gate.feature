Feature: Onboarder slice 2 - survey to agreed contract through the existing gate
  ...
  # onboarding-contract-gate.js. No CLI is reimplemented; the onboarder
  # chains them and narrates. The agreed contract is committed back to the
  # target repo on GitHub. Epic swarm-intelligence-layer.

  Background:
    Given an onboarder with a target onboarding in state "prerequisites-ready"
  ...
  Scenario: the survey phase clones the target repo and posts the proposed contract
    ...
    Then the onboarder clones the target repo using its own GitHub access
    And the survey runs against the onboarder's clone
  ...
  Scenario: the contract-agreed transition is proven by the existing fail-closed gate
    ...
    When the onboarder checks the build-start gate
  ...
  Scenario: the agreed contract is committed back to the target repo on GitHub
    ...
    And the onboarder posts the commit reference into the topic
  ...
  Scenario: a failed clone or survey holds the onboarding with a visible reason
    ...
    And the onboarder posts the failure reason and the retry instruction
