Feature: BL-1677 A property file's own prefix sweep never reaps a live peer's fixture
  ...
  Four property files sweep every os.tmpdir() entry sharing their prefix
  ...
  Scenario Outline: the owner-aware sweep reaps only roots whose recorded owner is gone
    ...
    When the prefix sweep the four files now use runs from this process
  ...
  # BL-1677 the-guard-census-carries-the-four-files-03
  # Census pin (BL-1445): the migrated set is named, not derived.
  Scenario: the BL-1623 guard's migrated census names the four files
    When the blind temp-dir sweep guard test runs
    Then it reports every test passed
    And its migrated census lists exactly eleven files including bl1354SharedPathLandedSiblingInvariants, bl1389UnlandedSiblingPathNeverRidesInvariants, bl1380ExpediteNeverAnswersUnshownQuestion and bl1239SuiteManifestAccountsForEveryTestFile
