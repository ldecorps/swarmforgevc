...
Scenario: burndown and velocity charts render from the endpoint
  ...
  Then a burndown chart per milestone and a velocity chart render from the endpoint's JSON
...
Scenario: the UI computes no metrics itself
  ...
  Then every value shown comes from the endpoint's JSON, with no computation in the UI
