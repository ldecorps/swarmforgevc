Feature: Onboarder slice 3 - prompts, launch handoff, and topic reuse
  ...
  # ready-to-launch handoff - the onboarder states the EXACT command the
  ...
  Background:
    Given an onboarder with a target onboarding in state "contract-agreed"
  ...
  Scenario: ready-to-launch states the exact launch command for the target host
    ...
    When the onboarder posts the launch handoff
  ...
  Scenario: the onboarder never claims to have launched the target swarm
    Given the onboarding is in state "ready-to-launch"
    When the principal asks whether the swarm is running
    Then the onboarder states it cannot launch or observe the target host
    And the onboarder restates the launch command instead
  ...
  Scenario: confirming the launch marks the onboarding done
    ...
    And the onboarder posts a completion summary naming the target
  ...
  Scenario: the next onboarding reuses the same topic with its own per-target state
    ...
    And the onboarder's replies name which target they concern
