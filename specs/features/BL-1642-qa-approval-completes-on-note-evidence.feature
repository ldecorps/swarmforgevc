Feature: BL-1642 A QA approval completes on note evidence without a no-op reason
  ...
  Scenario Outline: completion of a forwarding parcel held by <role> depends on what it has sent since dequeue and what it says
    ...
    Examples:
      ...
      | QA        | a forwarding git_handoff for BL-4242 in in_process | a note to the coordinator naming BL-4242 created before the dequeue sits in its sent mailbox | with no reason | refused naming BL-4242 and the two ways out, with nothing moved        |
