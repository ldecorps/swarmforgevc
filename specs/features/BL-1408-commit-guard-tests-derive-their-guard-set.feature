Feature: BL-1408 The commit-guard tests derive their guard set from the chain they exercise
  ...
  Three tests still pin the commit-guard chain against a list written by
  ...
  hand on the day a guard joined. BL-1398 and BL-1401 fixed two other copies
  of the same list; this feature is that no copy remains: each test reads the
  runner or hook it exercises through BL-1398's helper, so a guard added to a
  chain is stubbed and run without editing any test.
