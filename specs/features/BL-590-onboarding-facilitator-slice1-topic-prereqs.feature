Feature: Onboarder slice 1 - the Onboarding topic and the prerequisites phase
  ...
  # the other standing topics, inbound routed to the onboarder - never
  # the front-desk SUP path), a supervised onboarder poll-loop
  ...
  Background:
    Given an onboarder bound to the primary group's Onboarding topic with a controllable clock

  # BL-590 onboarding-topic-ensured-and-routed-01
  Scenario: the Onboarding topic is ensured with its own reserved subject and routed to the onboarder
    Given no Onboarding topic exists in the primary group
    When the onboarder service starts
    ...
    And a principal message in that topic reaches the onboarder
    And it never reaches the front-desk operator path

  # BL-590 new-onboarding-starts-at-prerequisites-02
  Scenario: giving the onboarder a repo URL opens a per-target state at the prerequisites phase
    ...
    And the onboarder posts where the onboarding stands and the first prerequisite instruction
  ...
  Scenario: a prerequisite advances only on a passing pasted verification
    ...
    And the onboarder posts the next prerequisite instruction
  ...
  Scenario: a bare done claim never advances a prerequisite
    ...
    And the onboarder re-asks for the step's verification command output
  ...
  Scenario: a failing verification keeps the step and explains the failure
    ...
    Then the step is not recorded as verified
    And the onboarder explains what failed and re-issues the exact instruction
  ...
  Scenario Outline: every prerequisite step is guided with an instruction and a verification
    ...
    When the onboarder posts the step's guidance
  ...
  Scenario: an onboarder restart resumes the persisted state instead of restarting
    Given the onboarding is on the "fork-clone" prerequisite step with two steps verified
    When the onboarder service restarts
  ...
  Scenario: the principal can pause and later resume the onboarding
    ...
    Then the onboarder holds and confirms the onboarding is paused
    When the principal posts the proceed control
    Then the onboarder resumes at the same step with the same instruction
  ...
  Scenario: all prerequisites verified advances to prerequisites-ready and names what comes next
    ...
    And the onboarder announces the survey phase comes next
