...
**Gherkin scenario recertification (partial — inbound email intake not yet wired):** A third published artifact, `recert-batch.json`, drip-feeds one oldest-reviewed-first Gherkin scenario at a time (only scenarios carrying a BL-111 stable `# TICKET-ID slug` tag are trackable; untagged ones are excluded, never mistracked under a position that could shift) into a "Recertify a scenario" section of the PWA. Reviewing from the phone offers three outcomes, each composing a `mailto:` link since the PWA is fully static with no live backend to call: **Confirm** (still accurate — just advances the scenario's last-reviewed timestamp), **Update** (propose new scenario text), and **Delete** (gated behind an explicit in-app confirmation screen before any send link exists). Confirm applies directly since it changes no content; Update/Delete are real contract changes and are meant to land as durable, one-per-change proposals for specifier review — the same pattern `rule_proposal` already uses — via a Resend Inbound webhook. **That webhook does not exist yet** (deferred by BL-073, contrary to earlier scoping that assumed it was already available): `parseRecertEmail`/`RecertProposal` are fully built and tested against doubles, but nothing public receives real mail today, and the PWA's `RECERT_EMAIL_TO` address is a placeholder (an `.invalid`-TLD address, per RFC 2606 — guaranteed non-deliverable) until that infrastructure lands. Applying an *accepted* update/delete proposal back into its `.feature` file also has no tool yet. In short: selection, confirm, and the compose/parse logic are real and tested; the inbound loop that turns a tapped Update/Delete into an applied `.feature` file change is not yet connected end to end.
