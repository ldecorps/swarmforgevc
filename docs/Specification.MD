...
**Bilingual French/English:** The dashboard has a visible FR/EN toggle. English is always the first-launch default regardless of browser locale; switching re-renders every already-fetched section instantly from cached data — no reload, no re-fetch — and the choice persists across visits via the same Cache Storage instance the service worker already owns (never `localStorage`). Every translatable field (vision doc markdown, ticket title/description, Gherkin scenario text) carries an additive `*Fr`/`*FrUntranslated` sibling on `backlog.json`/`docs-tree.json` (schema docs above), machine-translated by a CI-run pass (a DeepL adapter, with a null/pass-through fallback — publishing never fails or blocks on translation) whose content-hash cache is committed to `docs/i18n/translation-cache.json` so an unchanged string is never re-translated and builds stay reproducible. `*FrUntranslated: true` marks a field that's still showing English because translation was unavailable or failed for it — never a silently wrong label. Gherkin scenario text always shows the canonical English by default in either locale (the binding acceptance contract), with the French rendering one tap away. Operationally: the Action needs an `MT_API_KEY` secret (DeepL) to actually translate — configured as a GitHub Actions secret, never in the client bundle or the repo.
