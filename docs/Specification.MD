...
Still open: the actual serverless deployment (a thin platform handler in front of the core, plus a live Resend Inbound domain actually pointed at it) and swapping the PWA's `RECERT_EMAIL_TO` placeholder (an `.invalid`-TLD address, per RFC 2606 — guaranteed non-deliverable) for the real receiving address are deployment/operator steps, not yet done. Applying an *accepted* update/delete proposal back into its `.feature` file also has no tool yet (out of scope for BL-217; tracked under BL-150 + the existing `rule_proposal` flow).
