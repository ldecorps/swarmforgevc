...
5. **(BL-1130)** Before starting a merge, merge-tree foresight refuses a
   ...
   **(BL-1120, refined by BL-1386)** If `MERGE_HEAD` already existed, the
   ...
   rematch/reset as if the abort had succeeded.
...
| index not clean, not reconciled (BL-1653) | A **staged** path (`git diff --cached --name-only`) blocks the merge before it is even attempted — `git merge` refuses unconditionally whenever the index differs from `HEAD`, regardless of overlap with the incoming content. Checked BEFORE step 3's merge, separately from the dirty-overlap check above (unstaged/untracked collision) and from a real content conflict below: a staged path is `master-main-reconcile-lib/staged-index-block-reason`'s own reason, never folded into `:conflict`. Surfaces as `index-not-clean` in the daemon log (`staged path(s) block the absorb merge: <paths>`) and in the coordinator's note (`BL-891: master main <N> behind origin, index not clean: <path or count>`), so step 0's action names what to clear instead of the generic "absorb refused" text. Typically a writer's commit was abandoned mid-hook (killed at `daemon_cycle_guard_lib`'s 60 s subprocess bound, refused by a guard, or refused by git's own lock) and left its `git add` staged — `commit_integrity_lib.bb` and `briefing_email_lib.bb`'s `commit-sent-marker!` now `git restore --staged` their own paths on any such failure, so this outcome should be rare going forward; it still fires for any OTHER writer or a human's own leftover stage. |
...
| abort found no MERGE_HEAD, ownership released (BL-1653) | The abort itself answered "There is no merge to abort (MERGE_HEAD missing)" — with no `MERGE_HEAD` there is nothing left to abort, so retaining the ownership record (as the generic `abort failed` row above does) would repeat this same outcome forever with the owner file never clearing on its own. Distinct outcome, `merge-abort-no-merge-head`: ownership is released at once (`clear-merge-owner!`) and the NEXT tick starts fresh from the index-not-clean check above, rather than retrying an abort that can never succeed. |
