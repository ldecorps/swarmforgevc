...
| Storage | Host-persisted via BL-1153 (`.swarmforge/operator/web-ui-font-size-preferences.json`) — no webview `localStorage` / `sessionStorage` (Architecture Rule 3). See [BL-1153 how-to](BL-1153-sticky-web-font-size-choice.md). |
...
Since BL-1153, a full Mini App reload restores the last chosen size (default
13px only when no preference is stored).
