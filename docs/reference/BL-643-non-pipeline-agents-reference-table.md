...
| Operator Runtime Watch | supervisory | [`swarmforge/scripts/launch_operator_runtime_supervisor.sh`](../../swarmforge/scripts/launch_operator_runtime_supervisor.sh) (BL-993 — the always-on watch that restarts a dead `operator_runtime.bb` through [`swarmforge/scripts/start_operator_runtime.sh`](../../swarmforge/scripts/start_operator_runtime.sh), honoring deliberate stops) | [`stop_ancillary_services.sh`](../../swarmforge/scripts/stop_ancillary_services.sh)'s `stop_operator_runtime()` (stops the watch first, then the runtime it supervises) | — none — (a Babashka supervisor daemon, [`swarmforge/scripts/operator_runtime_supervisor.bb`](../../swarmforge/scripts/operator_runtime_supervisor.bb), not an LLM role) | `.swarmforge/operator/operator-runtime-supervisor.log` | `start_ancillary_services.sh` |
...
| Front Desk | transport | [`swarmforge/scripts/launch_front_desk.sh`](../../swarmforge/scripts/launch_front_desk.sh) | [`stop_ancillary_services.sh`](../../swarmforge/scripts/stop_ancillary_services.sh)'s `stop_front_desk()` | — none — see the naming-collision note below | `.swarmforge/operator/front-desk-supervisor.log` (supervisor lifecycle events) + `.swarmforge/operator/front-desk-diagnostics.log` (BL-582 — the bot's own durable diagnostic sink: callback drops, no-op approval records, and repaint failures; append-only, bounded to one rotation generation) | `start_ancillary_services.sh` (Telegram-env gated) |
