...
- [Nightly cooldown window (and control-pause state)](how-to/BL-617-nightly-cooldown-window.md) — scheduler vs manual pause, the shared `.swarmforge/operator/control-pause.json` marker, and (BL-1106) why `effective_backlog_depth_cli` must print the same paused cap from master and from any linked worktree.
...
- [Ticket land without operator absorb merge](how-to/BL-1131-ticket-land-without-operator-absorb-merge.md) — rematch-then-FF; `behind=0` / proceed without Complete-origin/main-merge (BL-1131).
