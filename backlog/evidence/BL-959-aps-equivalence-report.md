...
| `3a1d7b063` | Add default parameter inference to the Gherkin parser | **behavior change** | ✓ confirmed, and it is ON BY DEFAULT: `parse-file` 1-arity and the CLI both infer unless `--do-not-infer` is passed. New `aps.inference` namespace; literal step values become `pN` placeholders merged into example tables. This changes the IR every consumer reads (see §3). Also relevant: candidate `write-json!` adds `strip-empty-keys` — IR bytes differ even where semantics do not (the matrix compares gate outcomes and finding sets, not bytes, by design). |
