...
object HandsFreeReArmGate {
    ...
    /** Room / speaker tail after the player reports done. */
    const val DEFAULT_QUIET_TAIL_MS = 700L

    /**
     * Upper bound once playback has gone quiet. Past this with no further
     * audio, arm anyway rather than risk hands-free latching silently off.
     * Must stay well above [DEFAULT_QUIET_TAIL_MS].
     */
    const val DEFAULT_CEILING_MS = 8_000L

    /**
     * Absolute bound while [Input.playbackActive] stays true (slow TTS /
     * stuck OEM isSpeaking). Shorter ceilings must NOT arm onto live speech —
     * that is the voix-lente self-listen loop. Only this longer bound may.
     */
    const val DEFAULT_ACTIVE_PLAYBACK_CEILING_MS = 60_000L
    ...
    const val POST_ARM_SETTLE_MS = 250L
    ...
    data class Input(
        ...
        val activePlaybackCeilingMs: Long = DEFAULT_ACTIVE_PLAYBACK_CEILING_MS,
    ...
    fun decide(input: Input): Decision {
        with(input) {
            ...
            val quietCeilingAt = waitStartedAt + ceilingMs
            val activeCeilingAt = waitStartedAt + activePlaybackCeilingMs
            if (playbackActive) {
                // Never use the short quiet-tail ceiling while audio is still
                // reported active — slow TTS routinely outlives a premature
                // onDone and a 4–8s arm would open the mic onto Bubble's voice.
                if (nowMs >= activeCeilingAt) {
                    return Decision.Arm(
                        "active-playback ceiling (${activePlaybackCeilingMs}ms) reached — arming defensively"
                    )
                }
                return Decision.NotYet(
                    "playback still reported active",
                    recheckAtMs = (nowMs + pollIntervalMs).coerceAtMost(activeCeilingAt)
                )
            }
            if (nowMs >= quietCeilingAt) {
            ...
            if (quietFor < quietTailMs) {
                return Decision.NotYet(
                    ...
                    recheckAtMs = (lastAudioActivityAt + quietTailMs).coerceAtMost(quietCeilingAt)
                )
            }
