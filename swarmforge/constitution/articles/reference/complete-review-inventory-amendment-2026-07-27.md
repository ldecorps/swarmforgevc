...
## 10. Article 4.4 — pre-third-trim wording (BL-858 further split)

`04_quality_gates.md` Article 4.4's own text, verbatim, before BL-858's
third compression pass:

> - A reviewing role (cleaner, architect, hardender, documenter, QA) never
>   sends a parcel back at the FIRST defect it sees (**first-failure stop**,
>   BL-590: seven architect send-backs, one slice) — it finishes its own full
>   checklist and sends **one** bounce carrying **every** defect found.
> - **Complete means run-or-blocked, never assumed-clean.** A check blocked by
>   an earlier defect (unit behind a compile failure, mutation behind a red
>   suite) is recorded BLOCKED BY that defect — never passing, never omitted.
> - **One evidence file, one inventory**, items `D1..Dn` each with **class**
>   (`compile|unit|integration|acceptance|behavior|invariant-unencoded|spec-gap`),
>   **blamed role** (4.3 ownership), and a **remediation pointer**. A clean
>   sweep records an explicit NONE.
> - **One bounce, many owners**: goes to the EARLIEST blamed role (4.3), travels
>   with the parcel, and each later stage clears the items blamed on IT.
> - **Spec gaps leave by `note`** (priority `00`, to specifier AND coordinator —
>   the specifier is never sent a parcel, Article 1.2); an all-spec-gap
>   inventory bounces nothing.
> - **Unchanged**: BL-532 sibling deferral; `rule_proposal` never splits a
>   bounce. **A genuinely new bounce is still allowed** when a fix introduces
>   new defects or an unblocked check then fails.

## 8. Article 4.4's remaining bullets — Full Text (BL-858 further split)

`04_quality_gates.md` Article 4.4's own text, verbatim, before BL-858's
second trim pass compressed it further:
...
## 7. BL-536 (2026-08-04) — Full Text (BL-858 split)

Article 4.4's own bullet, verbatim, before BL-858 compressed it to a pointer:
