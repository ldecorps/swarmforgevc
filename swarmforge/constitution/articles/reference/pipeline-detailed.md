# PIPELINE.md — detailed elaboration (BL-858)

Not inlined into the boot prefix (BL-519: only top-level `articles/` files and
`swarmforge/PIPELINE.md` itself are). Read this on demand when your task
touches mono-router rotation mechanics, the QA merge-up stranding edge case,
or the expeditor. `swarmforge/PIPELINE.md` keeps every normative rule from the
sections below; this file holds the mechanics and rationale that moved out to
...
## Same gates, no machinery: the expeditor (BL-567) — full mechanics
...
## Roles/worktrees bullets, How-a-parcel-moves steps 1-4, Mono-router intro/steps — pre-third-trim wording (BL-858 further split)

`PIPELINE.md`'s own text, verbatim, before BL-858's third pass:
...
> Mono-router packs (`config rotation sequential`) keep **one resident** process
...
## Notify chain intro paragraph — Full Text (BL-858 second trim)

`PIPELINE.md`'s own text, verbatim, before BL-858's second pass pointed at
`01_roles.md` instead of restating (the coordinator/specifier duties below
are ALSO stated in Article 1.1/1.2, same boot prefix — this snapshot exists
so the exact PIPELINE-flavored wording, including the parcel-flow framing
words absent from Article 1, stays retrievable):
...
## How a parcel moves, steps 5-7 — Full Text (BL-858 second trim)
...
>      human intervenes; handoffd may nudge the coordinator to promote+route,
>      but the coordinator still owns it.
...
## Mono-router subsections — pre-second-trim wording (BL-858 further split)

`PIPELINE.md`'s own text, verbatim, before BL-858's second pass compressed
these three subsections to one line each:

> ### Aged-note rotation (BL-576)
>
> A solo `note` to a dormant role stays non-actionable for
> `note_actionable_after_ms` (default 20 min) to prevent broadcast thrash on a
> five-role merge-up; once aged in, the chase sweep rotates the resident to
> drain it. Fresh notes mid-parcel are unaffected; the age clock is the parcel
> header, never file mtime. Full key mechanics (delivery-wake suppression,
> newest-wins, one-rotation-per-sweep) and tuning: **reference/pipeline-detailed.md**,
> `docs/how-to/BL-576-aged-note-actionability-mono-router.md`.
>
> ### `rule_proposal` is immediately actionable, chase redirects to preferred (BL-795)
>
> Unlike a plain `note`, a directed `rule_proposal` (Article 5.1) is actionable
> the moment it lands — it never waits out `note_actionable_after_ms`, since it
> targets one role, not a broadcast. A chase poke landing on a non-preferred
> role now redirects to the preferred one; a stuck-holder alert keeps retrying
> rather than giving up. Full mechanics: **reference/pipeline-detailed.md**,
> `swarmforge/handoff-protocol.md`.
>
> ### Non-home role after QA merge-up (BL-550)
>
> QA's merge-up `note` broadcasts to **all five** pipeline worktree roles; a
...
`PIPELINE.md`'s own text, verbatim, before this file's second trim pass
tightened the wording in place (no relocation, just fewer words — these
snapshots exist so the exact prior phrasing, including specific facts cut
for brevity, stays retrievable).

### Mono-router idle and open slots — pack name examples
...
### How a parcel moves, step 6 — the nudge message string
...
### Endless-loop hard stop — original wording
