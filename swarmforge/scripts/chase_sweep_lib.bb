...
;; BL-1479: `*file*` is a dynamic var read at whatever moment code actually
;; runs, not captured at a function's own definition site - a function
;; body that reads `*file*` directly answers with whatever file the
;; CALLER happens to be reading at call time (measured live: park-ticket!
;; below resolved commit_integrity_cli.bb relative to the calling test
;; script's own directory, not this file's, the moment it was called from
;; a separate .bb script rather than via `bb -e`). Captured ONCE here, at
;; THIS file's own top-level load time - every sibling load-file call
;; above/below already depends on exactly this same correct resolution,
;; this just names it for a function body to reuse safely.
(def own-script-dir (str (fs/parent (fs/canonicalize *file*))))
...
;; BL-1652: pane-busy?/lane-running? are read ONCE per role per sweep (never
;; per item - see sweep-role-inbox! below) and gate ONLY the
;; already-at-ceiling branch: a role below maxChases already answers
;; "chased" regardless, so busy/lane never need to be consulted there. A
;; role at the ceiling whose own pane shows the busy footer or whose
;; worktree has a lane running is chased (backed off exactly like any other
;; "chased" item downstream) instead of respawned or dead-lettered - a
;; stale heartbeat alone is never sufficient when either live reading says
;; otherwise.
(defn decide-stale-item-action [chase-count config liveness pane-busy? lane-running?]
  (cond
    (< chase-count (:maxChases config)) "chased"
    (or pane-busy? lane-running?) "chased"
    (unresponsive-liveness? liveness) "respawned"
    :else "dead-lettered"))
...
;; BL-1652: `readings` (nil for every action but "respawned") carries the
;; per-sweep :liveness/:heartbeatAgeS/:activityAgeS/:busy/:lane snapshot
;; sweep-role-inbox! already gathered to DECIDE the action - never
;; recomputed here, so the log/telemetry line can never disagree with what
;; the decision actually saw.
(defn- apply-inbox-item-action! [role item action adapters now-ms readings]
  (case action
    ...
    "respawned" (let [handoff-id-str (handoff-id (:filePath item))]
                  ((:trigger-respawn! adapters) role (assoc readings :itemId handoff-id-str))
                  ((:log-telemetry! adapters)
                   (merge {:type "respawn" :role role :handoffId handoff-id-str :count (:chaseCount item)} readings)
                   now-ms))
...
;; behaving exactly as before (busy?/lane? read false, heartbeat age reads
;; nil).
(defn- optional-adapter-call [adapters k role]
  (when-let [f (k adapters)] (f role)))
...
(defn sweep-role-inbox! [role inbox-new-dir completed-dir abandoned-dir now-ms config adapters]
  ...
  (let [items (scan-inbox-new inbox-new-dir)
        ...
        ;; BL-1652: read ONCE per role per sweep, never per item - reuses
        ;; :role-agent-busy? (the SAME footer reading the claim-progress
        ;; check and chase-poke-and-notify! already key on, never a second
        ;; notion of busy).
        pane-busy? (boolean (optional-adapter-call adapters :role-agent-busy? role))
        lane-running? (boolean (optional-adapter-call adapters :role-lane-running? role))
        heartbeat-age-s (optional-adapter-call adapters :get-heartbeat-age-seconds role)
        ;; nil-safe: a fixture whose every item is already-terminal (reaped
        ;; before any branch below ever reads activity) never needed
        ;; last-activity-ms to resolve to a real number before BL-1652 -
        ;; base-readings is built unconditionally now (once per sweep,
        ;; never per item), so it must stay just as tolerant.
        activity-age-s (when (number? last-activity-ms) (/ (- now-ms last-activity-ms) 1000.0))
        ...
        ;; file write below only ever protects the NEXT sweep, never a
        ;; sibling item already mid-doseq in THIS one) and each independently
        ;; triggered a real respawn: the 2026-09-19 incident (QA respawned
        ;; seven times within 350ms - do-respawn!'s own busy precheck reads
        ;; not-busy on every one of those calls because the pane a tmux
        ;; respawn-pane -k just killed has not yet booted Claude far enough
        ;; to paint the busy footer before the NEXT item's decision runs).
    ...
    (doseq [item items]
      (let [already-terminal? (handoff-lib/already-terminal?
            ...
            action (cond
                     ...
                     ;; A sibling item already claimed this sweep's one
                     ;; respawn: this item does NOTHING this tick (never
                     ;; "chased" either - BL-1652 invariant 2 keeps its
                     ;; count and sidecars byte-identical, not merely
                     ;; unrespawned).
...
;; ── BL-1479: parked-active sweep ────────────────────────────────────────
;; active_backlog_max_depth counts every YAML in backlog/active/, and
;; nothing moves one back out once it is there - a ticket that becomes
;; unworkable after promotion (status: blocked, or a not_before the
;; promotion gates would now refuse) holds its slot until a human notices.
;; Reuses parked-ticket? (status) and promotion-gates-lib/not-before-
;; refusal (BL-1469, already load-filed above) rather than a second
;; regex/date parse of either (BL-897).

(defn- read-park-candidate
  "One backlog/active/*.yaml as {:id :status :content :file} - :content is
   kept (unlike read-active-item's own shape above) because
   promotion-gates-lib/not-before-refusal reads not_before straight off
   the raw ticket text, never a second field extractor. :file is always a
   plain string (never a raw fs/path object), so this shape round-trips
   through JSON unchanged for a caller across a language boundary
   (specs/pipeline/steps/bl1479ParkedActiveSweepSteps.js)."
  [yaml-file]
  (let [content (slurp (str yaml-file))]
    {:id (read-yaml-field content "id")
     :status (read-yaml-field content "status")
...
(defn read-park-candidates
  "Every backlog/active/*.yaml with an id, as read-park-candidate. Same
   fail-open-on-missing-dir posture as read-active-items."
  [active-dir]
  (if-not (fs/exists? active-dir)
    []
    (->> (fs/list-dir active-dir)
         (filter #(str/ends-with? (fs/file-name %) ".yaml"))
         (map read-park-candidate)
         (filter :id)
         vec)))

(defn parked-active-condition
  "nil when the ticket can advance under the ordinary promotion rules;
   otherwise the human-readable condition string a park's note/commit
   subject names. status: blocked is checked first (BL-1145/BL-1301's own
   park value, and the ticket's own out-of-scope: this sweep adds no
   auto-clearing of it); otherwise promotion-gates-lib/not-before-
   refusal's own reason text (e.g. \"not_before: 2026-09-10 is 3 days
   away\") - reused verbatim, never restated, so the sweep and the
   promotion gate can never name two different reasons for the same
   ticket. today is a caller-injected YYYY-MM-DD string; nothing here
   reads a clock."
  [status content today]
  (if (parked-ticket? status)
    dropped-parcel-park-status
    (:reason (promotion-gates-lib/not-before-refusal content today))))

(defn parked-active-items
  "candidates: read-park-candidates' own shape. live-ticket-ids: the set
   of ids with a parcel in ANY role's :new/:in_process mailbox right now
   (caller-injected - the same live-mail-trail-dirs scope
   dropped-parcel-evaluation uses one door up, never a second definition
   of \"a parcel is in flight\"). today: caller-injected YYYY-MM-DD.

   Returns {:to-park [{:id :file :condition}] :refused [{:id :condition}]}
   - a ticket with no condition (can advance) appears in neither set, and
   a ticket with BOTH a condition and live mail is refused, never parked
   (invariant 3: a parcel anywhere keeps the ticket in active/, whatever
   its status)."
  [candidates live-ticket-ids today]
  (reduce (fn [acc {:keys [id status content file]}]
            (if-let [condition (parked-active-condition status content today)]
              (if (contains? live-ticket-ids id)
                (update acc :refused conj {:id id :condition condition})
                (update acc :to-park conj {:id id :file file :condition condition}))
              acc))
          {:to-park [] :refused []}
          candidates))

(defn park-commit-message [id condition]
  (str "Park " id ": active -> paused (" condition ")"))

(defn- park-ticket-rel-paths [file]
  (let [base (fs/file-name file)]
    {:active-rel (str "backlog/active/" base)
     :paused-rel (str "backlog/paused/" base)}))

;; BL-1390: never a mutating git command against a root that is not
;; genuinely this checkout's own git-common-dir - the fixture-URL-into-
;; live-remote incident this rule exists to prevent (measured live: a test
;; fixture's own git call resolved against the shared checkout because its
;; root argument was blank/wrong, and wrote into the real remote).
(defn- confirm-git-common-dir! [root]
...
(defn park-ticket!
  ...
  (if-not (confirm-git-common-dir! root)
    ...
    (let [{:keys [active-rel paused-rel]} (park-ticket-rel-paths file)
