...
;; ── BL-1081 QA bounce D1: the spike-seat token agrees across the boundary ─
(load-file (str (fs/path repo-root "swarmforge" "scripts" "prompt_engine_lib.bb")))
(let [r (process/sh ["node" "-e"
                     (str "const {ACP_SPIKE_SEAT_AGENT}=require("
                          (json/generate-string (str (fs/path out-dir "acpSeatLaunch.js")))
                          ");process.stdout.write(ACP_SPIKE_SEAT_AGENT);")])
      ts-seat (str/trim (str (:out r)))
      bb-seat prompt-engine-lib/acp-spike-seat-agent]
  (when-not (zero? (:exit r))
    (fail! (str "could not read ACP_SPIKE_SEAT_AGENT from compiled TS: " (:err r))))
  (when-not (= ts-seat bb-seat)
    (fail! (str "spike-seat agent disagrees across the boundary: TS " ts-seat " vs bb " bb-seat)))
  (when-not (prompt-engine-lib/acp-hosted-spike-seat? "vibe")
    (fail! "acp-hosted-spike-seat? must be true for vibe"))
  (when (prompt-engine-lib/acp-hosted-spike-seat? "gemini")
    (fail! "acp-hosted-spike-seat? must be false for other acp-native agents")))

(if (seq @failures)
  (do (doseq [f @failures] (binding [*out* *err*] (println f)))
      (println (str "\n" (count @failures) " failure(s)"))
      (System/exit 1))
  (println "bl1081_acp_snapshot_agreement_test_runner: ok - the TS writer and bb reader agree on payload, path, and spike seat"))
