#!/usr/bin/env bash
# Smoke tests for start_cursor_bridge.sh / stop_cursor_bridge.sh.
set -euo pipefail
set +m
...
make_fixture() {
  ...
  cp "$SRC/start_cursor_bridge.sh" "$SRC/stop_cursor_bridge.sh" "$d/swarmforge/scripts/"
...
PID_FILE_IN() { echo "$1/.swarmforge/operator/cursor-bridge.pid"; }

F="$(make_fixture)"
DRY="$(CURSOR_BRIDGE_LAUNCH_DRYRUN=1 bash "$(START_IN "$F")" "$F" 2>&1)"
check "start dry-run prints the node command" '[[ "$DRY" == *"DRYRUN start_cursor_bridge cmd:"* ]]'
check "start dry-run writes no pid file" '[[ ! -f "$(PID_FILE_IN "$F")" ]]'
rm -rf "$F"

F="$(make_fixture)"
rm -f "$F/extension/out/tools/telegram-cursor-bridge.js"
OUT="$(CURSOR_BRIDGE_BOT_TOKEN=x TELEGRAM_CHAT_ID=x TELEGRAM_PRINCIPAL_USER_ID=x CURSOR_API_KEY=x \
...
OUT="$(CURSOR_BRIDGE_BOT_TOKEN=x TELEGRAM_CHAT_ID=x TELEGRAM_PRINCIPAL_USER_ID=x CURSOR_API_KEY=x \
  bash "$(START_IN "$F")" "$F" 2>&1)" && rc=0 || rc=$?
check "start is idempotent when pid file points at a live process" \
  '[[ "$rc" -eq 0 && "$OUT" == *"already running"* ]]'
check "idempotent start leaves the existing process running" 'kill -0 "$LIVE_PID" 2>/dev/null'
...
check "stop removes the pid file" '[[ ! -f "$(PID_FILE_IN "$F")" ]]'
check "stop reports stopped" '[[ "$OUT" == *"Stopped cursor bridge"* ]]'
check "stop terminates the recorded process" '! kill -0 "$LIVE_PID" 2>/dev/null'
rm -rf "$F"
