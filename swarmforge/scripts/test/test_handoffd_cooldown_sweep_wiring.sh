...
wait_for_count "$ROOT/apply-cooldown-pause-calls.log" 2 25 \
  || fail "expected the sweep to fire on more than one poll cycle within 25s, got $(wc -l < "$ROOT/apply-cooldown-pause-calls.log" 2>/dev/null || echo 0) calls"
