...
# BL-1390's second incident: a blind prefix sweep deletes a CONCURRENT copy's
# fixtures. This suite is invoked once per scenario by its acceptance handler,
# so it runs concurrently too. fixture_isolation_begin bounds the clock, logs
...
CODE="extension/src/bl1391-fixture.ts"
...
setup() {
  ...
  # The bare repo's HEAD would otherwise be the init default, so a later
  # `clone -b main` races the first push - the architect's observed
  ...
  # scripts tree per run is heavy enough to race git's object writes.
  ...
  rmdir "$root/swarmforge/scripts" 2>/dev/null || true
  ln -s "$WORK/shared-scripts" "$root/swarmforge/scripts" || fail "setup($name): could not copy scripts from $REPO_ROOT"
  ...
  # A real TRACKED non-bookkeeping file for scenario 3. The scripts tree is a
  # symlink now (one copy per run), and git tracks the symlink, not the files
  # under it - so nothing there can conflict.
...
printf 'export const fixture = 2; // theirs\n' > "$other/$CODE"
gq "$other" commit -aqm "QA: rewrite the fixture source"; gq "$other" push origin main
ours_appends_notes
printf 'export const fixture = 3; // ours\n' > "$root/$CODE"
gq "$root" commit -aqm "specifier: rewrite the same source line"
run_tick
