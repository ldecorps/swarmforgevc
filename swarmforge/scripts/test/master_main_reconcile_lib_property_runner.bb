...
;; ── BL-1386's three DECLARED invariants (coder-authored first, BL-654) ────
;;
;;   1. No sweep tick ends with a MERGE_HEAD the daemon itself created unless
;;      an ownership record naming that MERGE_HEAD sha exists on disk.
;;   2. The daemon aborts only a merge it can PROVE it started: a MERGE_HEAD
;;      whose sha matches its own record. No record, or a different sha, is
;;      never aborted - BL-1120 stands in full for every foreign merge.
;;   3. A failed abort is never reported as an abort: the log names which of
;;      merge and abort failed and quotes git's own text, never the fixed
;;      word conflict when git reported no conflict.
;;
;; GENERATOR REACH, by construction rather than by draw. The whole defect
;; lives in ONE cell of the outcome grid - merge failed AND abort failed -
;; which is also the rarest cell if the two are drawn independently. So the
;; grid is enumerated: every (merge-outcome x abort-outcome) pair is built on
;; every run, and the run asserts each cell was reached. A draw could pass a
;; hundred times having never once failed an abort, which is exactly how the
;; live defect survived until a human read the index by hand.

(def ^:private bl1386-merge-outcomes
  [{:label "merge-ok" :res {:success true}}
   {:label "hook-refusal" :res {:success false :error "hook refused the merge" :conflict? false}}
   {:label "genuine-conflict" :res {:success false :error "CONFLICT (content): a.txt" :conflict? true}}
   {:label "silent-failure" :res {:success false :error "" :conflict? false}}])

(def ^:private bl1386-abort-outcomes
  [{:label "abort-ok" :res {:success true}}
   {:label "abort-locked" :res {:success false :error "fatal: Unable to create '.git/index.lock': File exists."}}
   {:label "abort-legacy-nil" :res nil}])

(defn- bl1386-run [merge-res abort-res]
  (let [events (atom [])
        result (master-main-reconcile-lib/absorb-with-merge!
                {:ff! (fn [] {:success false})
                 :merge! (fn [] merge-res)
                 :abort! (fn [] (swap! events conj [:abort]) abort-res)
                 :fallback! (fn [] (swap! events conj [:fallback]) {:success true :outcome :rematched})
                 :record-owner! (fn [] (swap! events conj [:record]))
                 :clear-owner! (fn [] (swap! events conj [:clear]))
                 :log! (fn [l t] (swap! events conj [:log l t]))})]
    {:result result :events @events}))

(def ^:private bl1386-reached (atom #{}))

(doseq [{m-label :label m-res :res} bl1386-merge-outcomes
        {a-label :label a-res :res} bl1386-abort-outcomes]
  (let [cell (str m-label "/" a-label)
        {:keys [result events]} (bl1386-run m-res a-res)
        logs (filter #(= :log (first %)) events)
        recorded? (some #(= [:record] %) events)
        cleared? (some #(= [:clear] %) events)
        aborted? (some #(= [:abort] %) events)
        fell-back? (some #(= [:fallback] %) events)
        merge-ok? (:success m-res)
        ;; nil is a pre-BL-1386 adapter's "no opinion", which was always
        ;; treated as success - the legacy contract, preserved.
        abort-ok? (or (nil? a-res) (not (false? (:success a-res))))]
    (swap! bl1386-reached conj cell)

    ;; Invariant 1: ownership is recorded BEFORE the merge can fail, always.
    (when-not recorded?
      (swap! failures conj (str "BL-1386 invariant 1 [" cell "]: the merge ran without recording ownership first")))
    ;; ...and the record is released if and only if nothing is left open.
    (let [should-clear? (or merge-ok? abort-ok?)]
      (when (and should-clear? (not cleared?))
        (swap! failures conj (str "BL-1386 invariant 1 [" cell "]: nothing is left open, yet the ownership record was not cleared")))
      (when (and (not should-clear?) cleared?)
        (swap! failures conj (str "BL-1386 invariant 1 [" cell "]: a MERGE_HEAD is still open, yet ownership was cleared - the next tick will call it a human's"))))

    ;; Invariant 2: the abort is only ever attempted on a merge THIS call
    ;; started, i.e. only after its own merge! failed. Never speculatively.
    (when (and merge-ok? aborted?)
      (swap! failures conj (str "BL-1386 invariant 2 [" cell "]: aborted a merge that had succeeded")))
    (when (and (not merge-ok?) (not aborted?))
      (swap! failures conj (str "BL-1386 invariant 2 [" cell "]: a failed merge was left without an abort attempt")))

    ;; Invariant 3: the log says which step failed, in git's own words, and
    ;; never calls something a conflict that git did not.
    (when-not merge-ok?
      (let [labels (set (map second logs))]
        (if abort-ok?
          (do
            (when (and (:conflict? m-res) (not (contains? labels "conflict")))
              (swap! failures conj (str "BL-1386 invariant 3 [" cell "]: a genuine conflict lost its BL-1214 conflict label")))
            (when (and (not (:conflict? m-res)) (contains? labels "conflict"))
              (swap! failures conj (str "BL-1386 invariant 3 [" cell "]: reported `conflict` for a failure git never called a conflict")))
            (when (and (not (:conflict? m-res)) (not (contains? labels "merge-failed")))
              (swap! failures conj (str "BL-1386 invariant 3 [" cell "]: a failed merge was not reported as merge-failed"))))
          (do
            (when-not (contains? labels "merge-abort-failed")
              (swap! failures conj (str "BL-1386 invariant 3 [" cell "]: a FAILED abort was not reported as merge-abort-failed")))
            (when (contains? labels "conflict")
              (swap! failures conj (str "BL-1386 invariant 3 [" cell "]: a failed abort was reported as a conflict")))
            ;; git's own text, not a fixed string.
            (let [abort-log (first (filter #(= "merge-abort-failed" (second %)) logs))]
              (when (and abort-log (seq (str (:error a-res)))
                         (not (clojure.string/includes? (str (nth abort-log 2)) (str (:error a-res)))))
                (swap! failures conj (str "BL-1386 invariant 3 [" cell "]: the failed-abort log does not quote git's error text"))))
            ;; and a still-open merge must never be rematched on top of.
            (when fell-back?
              (swap! failures conj (str "BL-1386 invariant 3 [" cell "]: fell through to rematch with the merge still open")))))))))

;; Generator reach, asserted rather than hoped for.
(let [expected (set (for [{m :label} bl1386-merge-outcomes
                          {a :label} bl1386-abort-outcomes]
                      (str m "/" a)))
      missing (clojure.set/difference expected @bl1386-reached)]
  (if (empty? missing)
    (println (str "BL-1386 generator reach: all " (count expected) " merge/abort cells exercised"))
    (do (println (str "BL-1386 GENERATOR REACH FAILURE, never exercised: " missing))
        (System/exit 1))))

;; Non-vacuity: the pre-BL-1386 ladder - which discarded abort!'s result and
;; fell through regardless - must be CAUGHT by invariant 3 above. Without
;; this, the rows would pass against the very code that orphaned three
;; merges on 2026-09-04.
(let [orphaning-events (atom [])
      _ (master-main-reconcile-lib/absorb-with-merge!
         {:ff! (fn [] {:success false})
          :merge! (fn [] {:success false :error "hook refused"})
          ;; the OLD adapter shape: result discarded by construction
          :abort! (fn [] {:success false :error "index.lock exists"})
          :fallback! (fn [] (swap! orphaning-events conj [:fallback]) {:success true :outcome :rematched})
          :record-owner! (fn [] nil)
          :clear-owner! (fn [] (swap! orphaning-events conj [:clear]))
          :log! (fn [l t] (swap! orphaning-events conj [:log l t]))})]
  (if (and (not (some #(= [:fallback] %) @orphaning-events))
           (not (some #(= [:clear] %) @orphaning-events))
           (some #(= "merge-abort-failed" (second %)) @orphaning-events))
