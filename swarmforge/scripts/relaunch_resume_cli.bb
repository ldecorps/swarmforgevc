...
(defn cmd-resolve-boot-role
  "Prints the resolved role name on stdout, and ONLY a real role name -
   never a bare 'nil' (Clojure's println of nil prints the 4 characters
   'nil', and the caller writes stdout verbatim into the durable
   mono-router-active-role marker; a literal 'nil' there would destroy the
   record this ticket exists to preserve). When no role can be resolved
   (empty/unreadable roles.tsv) or an unexpected exception escapes any of
   the reads above, stdout is left EMPTY and a loud line goes to stderr
   instead - the caller must treat empty stdout as 'do not write the
   marker', never write it literally."
  [root]
  (try
    (let [home (home-role-at root)
      ...
      (if (not (str/blank? (str role)))
        (println role)
        (binding [*out* *err*]
          (println "BL-648 LOUD: no home role could be resolved (roles.tsv missing or empty) - printing nothing; caller must not write this to the active-role marker."))))
    (catch Exception e
      (binding [*out* *err*]
...
(defn cmd-sweep
  "Never lets an unexpected exception escape to a non-zero exit: sweep!
   ...
   surfaced via the sweep's own log! lines), so this catch is the backstop
   for anything else - e.g. default-adapters itself failing to read the
   project's roles.tsv or tmux-socket file. Either way the launch must
   continue; a stranded claim is surfaced, never fatal."
  [root resumed-role]
  (try
    ...
    (catch Exception e
      (binding [*out* *err*]
        (println (str "BL-648 LOUD: orphan-claim sweep failed unexpectedly (" (.getMessage e)
                      ") - continuing launch with claims possibly still stranded; check .swarmforge/ manually."))))))
