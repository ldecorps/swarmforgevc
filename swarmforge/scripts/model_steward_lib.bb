...
(defn register-model
  "Adds or replaces a Model Registry entry. Status defaults to \"candidate\"
   ...
   Optional :known_limitations (BL-557) is a vector of short operator-facing
   limitation strings; absent means none recorded yet (compat-docs says so)."
  [registry provider model {:keys [status context_window cost_class underlying_name trace known_limitations]}]
  (assoc-in registry [:models (model-key provider model)]
            (cond-> {:provider provider
              ...
              known_limitations (assoc :known_limitations (vec known_limitations)))))
...
(defn seed-data->registry
  ...
  (reduce
   (fn [registry {:keys [provider model status context_window cost_class underlying_name trace known_limitations]}]
...
;; ── Compatibility docs (BL-557) ───────────────────────────────────────────
(defn- limitations-lines
  [entry]
  (let [xs (:known_limitations entry)]
    (if (seq xs)
      (mapv str xs)
      ["(none recorded)"])))

(defn- render-model-section
  [{:keys [provider model status] :as entry}]
  (str "### " provider "/" model "\n"
       "- **Status:** " status "\n"
       "- **Known limitations:**\n"
       (str/join "\n" (map #(str "  - " %) (limitations-lines entry)))
       "\n"))

(defn- render-role-matrix-section
  [registry]
  (let [roles (->> (:role_matrix registry) keys (map str) sort)]
    (str "## Role recommendation matrix\n\n"
         "Live projection of the Role Recommendation Matrix (certified models "
         "only, highest score first). Query the same data via "
         "`model-steward role-matrix <role>`.\n\n"
         (str/join
          "\n"
          (for [role roles
                :let [ranked (role-recommendations registry role)]
                :when (seq ranked)]
            (str "### " role "\n"
                 (str/join "\n"
                           (map-indexed
                            (fn [i {:keys [provider model score]}]
                              (str (inc i) ". " provider "/" model " (score " score ")"))
                            ranked))
                 "\n"))))))

(defn render-compat-docs
  "Pure markdown projection of registry state for operator-facing
   compatibility documentation (BL-557). Regenerated from the registry —
   never hand-authored content."
  [registry]
  (let [models (registry-summary registry)]
    (str "# Model compatibility\n\n"
         "> Generated by `model-steward compat-docs`. Do not hand-edit; "
         "regenerate from the Model Steward registry.\n\n"
         "## Models\n\n"
         "Each registered model with its certification status and known "
         "limitations. See also the [Role recommendation matrix]"
         "(#role-recommendation-matrix).\n\n"
