...
# MONO_ROUTER_BOOT_ROLE that launch_role reads for index 1.
#
# Neither bb call site below is bare: this script runs under
# `set -euo pipefail`, and a best-effort recovery step must never be able
# to abort a launch and leave the swarm fully dead - strictly worse than
# the stall this ticket exists to fix. relaunch_resume_cli.bb's own
# commands already catch internally and degrade rather than throw; these
# guards are the belt-and-suspenders backstop (a missing bb binary, the
# script file itself unreadable, etc.), and resolve-boot-role's output is
# validated against the known ROLE_INDEX before it is ever written to the
# durable marker - stdout that is empty or names an unknown role leaves the
# marker untouched rather than corrupting it.
resolve_and_sweep_relaunch_resume() {
  MONO_ROUTER_BOOT_ROLE=""
  if [[ "$ROTATION_MODE" == "router" ]]; then
    local resolved=""
    if ! resolved="$(bb "$SCRIPT_DIR/relaunch_resume_cli.bb" resolve-boot-role "$WORKING_DIR")"; then
      echo -e "${YELLOW}BL-648 LOUD: resolve-boot-role exited non-zero - booting resident at home, mono-router-active-role marker left untouched.${RESET}" >&2
      resolved=""
    fi
    if [[ -n "$resolved" && -n "${ROLE_INDEX[$resolved]:-}" ]]; then
      MONO_ROUTER_BOOT_ROLE="$resolved"
      mkdir -p "$STATE_DIR"
      printf '%s\n' "$MONO_ROUTER_BOOT_ROLE" > "$STATE_DIR/mono-router-active-role"
    elif [[ -n "$resolved" ]]; then
      echo -e "${YELLOW}BL-648 LOUD: resolve-boot-role printed unknown role '$resolved' - booting resident at home, mono-router-active-role marker left untouched.${RESET}" >&2
    fi
  fi
  echo -e "${CYAN}Orphan-claim sweep (BL-648)...${RESET}"
  if ! bb "$SCRIPT_DIR/relaunch_resume_cli.bb" sweep "$WORKING_DIR" "$MONO_ROUTER_BOOT_ROLE"; then
    echo -e "${YELLOW}BL-648 LOUD: orphan-claim sweep exited non-zero - continuing launch; claims may still be stranded, check .swarmforge/ manually.${RESET}" >&2
  fi
}
