  ...
  (let [conf-text (mono-router-conf-text)
    ...
    (if (mono-router-lib/rotate-home?
         {:rotation-router? (mono-router-lib/conf-rotation-router? conf-text)
          ...
          :home-role home-role
          :mailbox-empty? true})
      ;; Hotfix 2026-09-13: same shape as ready_for_next_task.bb - the
      ;; resident follows the parcel it just forwarded (ROTATE_TO); the
      ;; wrapper prefers ROTATE_TO over HOME_ROLE.
