#!/usr/bin/env bash
# Pipeline-only stop: tmux role agents, handoffd (+ supervisor), stale sockets,
# SwarmForge copilot processes, and swarm state markers. Does NOT stop
# ancillaries (operator runtime, Telegram front desk, onboarder, tunnels, or
# an operator-launched babysitterd outside .swarmforge/babysitter/).
# Idempotent — safe when nothing is running.
#
# Scope: pipeline-only (NOT full stack — use ./stop-swarm.sh for that).
#
# Usage:
#   kill_pipeline_swarm.sh [repo-root]
#   kill_pipeline_swarm.sh --sweep-inbox [repo-root]
#   kill_pipeline_swarm.sh --reset-worktrees [repo-root]
#   kill_pipeline_swarm.sh --full [repo-root]   # inbox sweep + worktree reset
#
# Legacy alias: kill_all_swarm.sh (shim — prefer this name).
set -euo pipefail
...
copilot_pids_for_root() {
  ...
  while IFS= read -r line; do
    ...
    [[ "$pid" =~ ^[0-9]+$ ]] || continue
    [[ "$rest" == *copilot* ]] || continue
    [[ "$rest" == *SwarmForge* ]] || continue
    [[ "$rest" == *"$root"* ]] || continue
    printf '%s\n' "$pid"
  done <<< "$ps_out"
}
