...
| `required_wiring` | list[string] | (BL-531) Declare required call sites that this ticket's code must wire. Each entry is `path::pattern` or `path::pattern::why`, split on the first two `::` occurrences (so `why` may contain `::`). The QA-edge gate checks that each `pattern` appears as a fixed string in `path` at the parcel commit. Paths are relative to repo root. Use this when your ticket builds a mechanism (e.g. a library, a CLI, a config handler) that only works when called from specific locations — declare those locations and let the gate enforce they landed before QA. Example: `"swarmforge/scripts/swarm_handoff.bb::pre_qa_gate_lib::wired into handoff validation"`. Reading is flow-style (`required_wiring: [entry1, entry2]`) or block-style (`required_wiring:` followed by `  - entry1` lines) — both work. |
| `abandoned_commits` | list[string] | (BL-531) Declare commits that are legitimately unmerged and should not trigger the QA-edge ancestry gate. Use when a ticket intentionally leaves work on a feature branch, or a commit is a discarded experiment. Each entry is a commit SHA abbreviation (10 hex characters is idiomatic but any valid prefix works). The ancestry check skips any commits matching an entry in this list. Example: `abandoned_commits: ["e57a237b", "f1e2d3c4b5"]`. Flow-style or block-style YAML list format. |
